...
public class FeishuImageProcessingJobService {
    ...
    public String submit(BotConfigEntity bot, String chatId, String messageId, String imageKey) {
        ...
        String processingMessageId = sendInitialProcessingCard(bot, chatId, traceId);
        try {
            visionTaskExecutor.execute(() -> run(bot, chatId, messageId, imageKey, traceId, processingMessageId));
        } catch (TaskRejectedException exception) {
            String message = "演示保护：OCR 队列已满，图片已收到但没有进入后台处理，请稍后重试。";
            visionPipelineService.markError(traceId, "VISION_QUEUE_FULL", message);
            replaceOrSendCard(bot, chatId, processingMessageId,
                    feishuCardService.buildErrorCard("图片排队失败", message, traceId));
        }
        return traceId;
    }

    private String sendInitialProcessingCard(BotConfigEntity bot, String chatId, String traceId) {
        try {
            return feishuClientAdapter.sendCard(
                    bot,
                    chatId,
                    feishuCardService.buildProcessingCard(traceId, "received", 5, "已收到图片，等待后台任务开始")
            );
        } catch (RuntimeException exception) {
            log.warn("发送飞书处理中卡片失败，继续保留 traceId={}：{}", traceId, exception.getMessage());
            return "";
        }
    }
    ...
    private void replaceOrSendCard(BotConfigEntity bot, String chatId, String processingMessageId, Map<String, Object> card) {
        ...
        try {
            ...
            log.warn("发送飞书结果卡片失败：{}", exception.getMessage());
        }
    }
}
